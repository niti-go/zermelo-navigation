#!/usr/bin/env python3
"""
SAC Online Training on OGBench Pointmaze
=========================================
...
Logs training metrics (actor loss, critic loss, entropy coefficient,
log probability) and evaluation metrics (success rate, return, episode
length) to wandb.

Usage:
  cd ~/online_training_ogbench
  conda activate flowrl
  CUDA_VISIBLE_DEVICES=0 python train_sac_online.py
  CUDA_VISIBLE_DEVICES=5 python train_sac_online.py --total_timesteps 2000000 --seed 42

# 1. Start tmux
tmux new -s sac_ogbench
conda activate flowrl
wandb login
# 6. Set GPU and run
CUDA_VISIBLE_DEVICES=0 python train_sac_online.py
# 7. Detach: Ctrl+b, then d
# 8. Reattach later: tmux attach -t sac_ogbench
"""
    ...
    def _on_step(self) -> bool:
        ...
        if step % self.eval_freq == 0 and step != self._last_log_step:
            ...
            if success > self.best_success:
                ...
                self.model.save(str(Path("SAC_online_ogbench") / "best_model"))
...
def main():
    parser = argparse.ArgumentParser(
        description="SAC online training on OGBench pointmaze"
    )
    ...
    out_dir = Path("SAC_online_ogbench")
    out_dir.mkdir(parents=True, exist_ok=True)

    run = wandb.init(
        project=args.project,
        entity=WANDB_ENTITY,
        name=f"SAC_online_{args.env_name}_seed{args.seed}",
        group="SAC",
        config=dict(
            algo="SAC", mode="online", env=args.env_name, seed=args.seed,
            total_timesteps=args.total_timesteps,
            learning_rate=args.learning_rate, batch_size=args.batch_size,
            gamma=args.gamma, tau=args.tau,
            learning_starts=args.learning_starts, ent_coef=args.ent_coef,
        ),
        reinit=True,
    )

    print(f"\n{'='*60}")
    print(f"  SAC / online  env={args.env_name}  seed={args.seed}")
