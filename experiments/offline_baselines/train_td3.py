#!/usr/bin/env python3
"""
TD3 Offline Baseline on AntMaze-UMaze-v2 (v2 — with diagnostics)
==================================================================
Trains ONLY TD3 on the D4RL offline antmaze-umaze-v2 dataset
for comparison with MeanFlowQL.

TD3 is an off-policy algorithm: the offline dataset is loaded into the SB3
replay buffer; gradient updates only, no env interaction.

v2 diagnostics:
  - tqdm progress bar
  - Training critic/actor loss + gradient norms logged to wandb every LOG_FREQ
  - Dataset sanity checks at startup
  - Replay buffer verification
  - Policy vs dataset action diagnostics at eval time

Usage:
  python train_td3_v2.py
  python train_td3_v2.py --configs config_1 --seed 42
"""
...
def fill_replay_buffer(model, dataset: dict) -> int:
    ...
    print(f"  First 100 obs match dataset: {'YES' if match else 'NO — BUFFER FILL BROKEN'}")
...
# ─────────────────────────────────────────────────────────────────────────────
# TD3 offline training — with diagnostics
# ─────────────────────────────────────────────────────────────────────────────

def train_offline_td3(
    config_name: str,
    hyperparams: dict,
    dataset: dict,
    project: str,
    ...
    """Offline training for TD3.

    Workflow:
      1. Create TD3 model (env only used for space definition).
      2. Fill replay buffer with the D4RL offline dataset (no env steps).
      3. Run TOTAL_GRADIENT_STEPS gradient updates from the static buffer.
      4. Evaluate on the live env every EVAL_FREQ steps.
      5. Record video and log everything to wandb.
    """
    out_dir = Path("TD3") / config_name
    out_dir.mkdir(parents=True, exist_ok=True)

    run = wandb.init(
        project=project,
        entity=WANDB_ENTITY,
        name=f"TD3_{config_name}_seed{seed}",
        group="TD3",
        config=dict(
            algo="TD3", config=config_name, env=ENV_NAME, seed=seed,
    ...
    print(f"\n{'='*60}")
    print(f"  TD3 / {config_name}  (offline off-policy)  seed={seed}")
    print(f"{'='*60}")
    ...
    pbar = tqdm(range(1, TOTAL_GRADIENT_STEPS + 1), desc="TD3", unit="step",
    ...
    for step in pbar:
        ...
        if step % LOG_FREQ == 0:
            metrics = extract_td3_metrics(model)
            ...
            postfix["g_crit"] = f"{grad_norms.get('grad_norm/critic', 0):.2f}"
            postfix["g_act"] = f"{grad_norms.get('grad_norm/actor', 0):.2f}"
...
def parse_args():
    p = argparse.ArgumentParser(
        description="TD3 offline baseline on antmaze-umaze-v2 — v2 with diagnostics"
    )
...
def main():
    ...
    for config_name in args.configs:
        if config_name not in CONFIGS:
            ...
            continue
        try:
            train_offline_td3(config_name, CONFIGS[config_name], dataset,
                              args.project, args.seed)
        except Exception:
            print(f"\nERROR during TD3/{config_name}:")
            traceback.print_exc()
