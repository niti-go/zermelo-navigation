#!/usr/bin/env python3
"""
TD3 Online Training on AntMaze-UMaze-v2
========================================
...
Logs training metrics (critic loss, actor loss, gradient norms) and
evaluation metrics (success rate, return, episode length) to wandb.

Usage:
  python train_td3_online.py
  python train_td3_online.py --total_timesteps 2000000 --seed 42
"""
    ...
    def _on_step(self) -> bool:
        ...
        if step % self.eval_freq == 0:
            ...
            if success > self.best_success:
                ...
                self.model.save(str(Path("TD3_online") / "best_model"))
...
def main():
    parser = argparse.ArgumentParser(
        description="TD3 online training on antmaze-umaze-v2"
    )
    ...
    out_dir = Path("TD3_online")
    out_dir.mkdir(parents=True, exist_ok=True)

    run = wandb.init(
        project=args.project,
        entity=WANDB_ENTITY,
        name=f"TD3_online_seed{args.seed}",
        group="TD3",
        config=dict(
            algo="TD3", mode="online", env=ENV_NAME, seed=args.seed,
    ...
    print(f"\n{'='*60}")
    print(f"  TD3 / online  seed={args.seed}")
    print(f"  total_timesteps={args.total_timesteps:,}")
    print(f"{'='*60}\n")

    train_env = make_train_env()

    # TD3 needs explicit exploration noise
