'''
Trains MeanFlowBC on an offline dataset of the Zermelo navigation environment.

MeanFlowBC is the critic-free ablation of MeanFlowQL: the identical MeanFlow
action generator (flow matching + JVP target + consistency loss), trained as
pure behavior cloning with no Q-function, no Bellman backup, and no
reward-guided policy improvement.  Use it to isolate the contribution of the
Q-learning half of MeanFlowQL.

=== FULL WORKFLOW ===
# (Identical to meanflowql_zermelo.py except step 5 trains MeanFlowBC.)

CUDA_VISIBLE_DEVICES=4 python ~/zermelo-navigation/scripts/meanflowbc_zermelo.py \
    --offline_steps=1000000 \
    --seed=0 \
    --save_interval=50000 \
    --proj_wandb=zermelo_hit_dynamic_poordataset \
    --run_group=meanflow_bc \
    --wandb_online=True
...
flags.DEFINE_string('run_group', 'meanflow_bc', 'Run group in wandb')
...
config_flags.DEFINE_config_file(
    'agent',
    os.path.join(_MEANFLOWQL_ROOT, 'agents', 'meanflow_bc.py'),
    lock_config=False,
)


# Keys MeanFlowBC needs from the .npz dataset.
_MFBC_KEYS = ('observations', 'next_observations', 'actions', 'rewards',
              'terminals', 'masks')


def load_zermelo_dataset_for_mfbc(dataset_path, reward_shift=0.0):
    """Load .npz and return (train_dict, val_dict) shaped for MeanFlowBC."""
    ...
    train_dict = tc.flatten_segments(data, train_segs, _MFBC_KEYS)
    val_dict = tc.flatten_segments(data, val_segs, _MFBC_KEYS)
...
def main(_):
    ...
    train_dict, val_dict = load_zermelo_dataset_for_mfbc(
    ...
    # BC training phase.
    bc_steps = FLAGS.offline_steps + FLAGS.online_steps
    best_metric_value = float('-inf') if maximize_metric else float('inf')
    patience_counter = 0
    early_stopped = False

    for i in tqdm.tqdm(range(pretrain_steps, bc_steps + pretrain_steps + 1),
                       smoothing=0.1, dynamic_ncols=True, desc="BC"):
        ...
        if FLAGS.eval_interval != 0 and (i % FLAGS.eval_interval == 0):
            ...
            if FLAGS.enable_early_stopping and FLAGS.early_stopping_metric in eval_metrics:
                ...
                if improved:
                    ...
                    patience_counter = 0
                    print(f"BC: New best {FLAGS.early_stopping_metric}: "
                          f"{current_metric:.4f}")
                else:
                    patience_counter += 1
                    print(f"BC: No improvement in {FLAGS.early_stopping_metric}. "
                          f"Patience: {patience_counter}/{FLAGS.early_stopping_patience}")

                if patience_counter >= FLAGS.early_stopping_patience:
                    print(f"Early stopping triggered during BC at step {i}")
                    early_stopped = True
